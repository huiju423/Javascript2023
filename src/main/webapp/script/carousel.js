...
function addEventToCarousel(carouselElem) {
	...
	//slide-left slide-right 두개이므로 for문을 사용하여 클릭 이벤트(createListenerSlide() 호출)
...
function createListenerSlide(carouselElem) {
	return function(event) {
		...
		//슬라이드 버튼 체크
		//className.includes("right") : right라는 글자를 포함되었으면
		//licount -1 : 5개일때 0,1,2,3,4 이므로 인덱스번호를 -1 처리해서 0,1,2,3
...
// window.addEventListener : 페이지가 완전히 로드된 후에 함수를 실행하는 방법
